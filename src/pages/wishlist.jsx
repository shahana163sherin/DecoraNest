...
const Wishlist = () => {
 ...
 const handleAddToCartFromWishlist = async (item) => {
  try {
    // 1️⃣ Add item to cart on backend
    const res = await addToCart(item.productId, 1);

    // 2️⃣ Update cart context (so UI updates instantly)
    addItemToCart(item); // use your CartContext function

    // 3️⃣ Optionally remove from wishlist
    // toggleWishlistItem(item); // removes if already in wishlist
